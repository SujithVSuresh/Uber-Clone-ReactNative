  ...
  return (
    <SafeAreaView style={[tw`bg-white flex-grow`, styles.container]}>
      ...
      renderItem={({ item: {id, title, multiplier, image }, item }) => (
        ...
        style={tw`flex-row justify-between items-center px-7 ${id === selected?.id && "bg-gray-200"}`}
        >
          <Image 
          style={{
            width: 100,
          ...
          <Text style={tw`text-xl`}>
            ...
            {Number(travelTimeInformation?.duration?.value * SURGE_CHARGE_RATE * multiplier / 100).toFixed(0)} Rs
